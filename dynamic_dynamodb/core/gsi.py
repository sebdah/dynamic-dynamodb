...
from dynamic_dynamodb.calculators import gsi as gsi_calc
...
def __ensure_provisioning_reads(table_name, table_key, gsi_name, gsi_key):
    ...
    if (consumed_read_units_percent == 0 and not
        ...
        if increase_reads_unit == 'percent':
            updated_provisioning = gsi_calc.increase_reads_in_percent(
                updated_read_units,
                increase_reads_with,
                table_name,
                table_key,
            ...
            updated_provisioning = gsi_calc.increase_reads_in_units(
                updated_read_units,
                increase_reads_with,
                table_name,
                table_key,
        ...
        if decrease_reads_unit == 'percent':
            ...
            updated_provisioning = gsi_calc.decrease_reads_in_units(
                updated_read_units,
                decrease_reads_with,
                table_name,
                table_key,
...
def __ensure_provisioning_writes(table_name, table_key, gsi_name, gsi_key):
    ...
    if (consumed_write_units_percent == 0 and not get_gsi_option(
        ...
        if increase_writes_unit == 'percent':
            updated_provisioning = gsi_calc.increase_writes_in_percent(
                updated_write_units,
                increase_writes_with,
                table_name,
                table_key,
            ...
            updated_provisioning = gsi_calc.increase_writes_in_units(
                updated_write_units,
                increase_writes_with,
                table_name,
                table_key,
        ...
        if decrease_writes_unit == 'percent':
            ...
            updated_provisioning = gsi_calc.decrease_writes_in_units(
                updated_write_units,
                decrease_writes_with,
                table_name,
                table_key,
