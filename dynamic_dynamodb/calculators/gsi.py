...
def get_min_provisioned_reads(current_provisioning, key_name):
    ...
    """
    if get_table_option(key_name, 'gsi_min_provisioned_reads'):
        return int(min(
            get_table_option(key_name, 'gsi_min_provisioned_reads'),
...
def get_min_provisioned_writes(current_provisioning, key_name):
    ...
    :param key_name: Name of the key
    """
    if get_table_option(key_name, 'gsi_min_provisioned_writes'):
        return int(min(
            get_table_option(key_name, 'gsi_min_provisioned_writes'),
            (current_provisioning * 2)))
...
def decrease_reads_in_percent(
        current_provisioning, percent, key_name, table_name, index_name):
    ...
    if min_provisioned_reads > 0:
        if updated_provisioning < min_provisioned_reads:
            logger.info(
                ...
                table_name, index_name, min_provisioned_reads))

            return min_provisioned_reads

    logger.debug(
        '{0} - GSI: {1} - '
        'Read provisioning will be decreased to {3:d} units'.format(
            table_name, index_name, updated_provisioning))
...
def increase_reads_in_percent(
    ...
    if get_table_option(key_name, 'gsi_max_provisioned_reads') > 0:
        if (updated_provisioning >
                get_table_option(key_name, 'gsi_max_provisioned_reads')):

            logger.info(
                '{0} - GSI: {1} - '
                'Reached provisioned reads max limit: {3:d}'.format(
                    table_name,
                    index_name,
                    int(get_table_option(
                        key_name, 'gsi_max_provisioned_reads'))))

            return get_table_option(key_name, 'gsi_max_provisioned_reads')

    logger.debug(
        '{0} - GSI: {1} - '
        'Read provisioning will be increased to {3:d} units'.format(
            table_name,
            index_name,
...
def decrease_writes_in_percent(
    ...
    if min_provisioned_writes > 0:
        if updated_provisioning < min_provisioned_writes:
            logger.info(
                '{0} - GSI: {1} - '
                'Reached provisioned writes min limit: {3:d}'.format(
                    table_name,
                    index_name,
    ...
    logger.debug(
        '{0} - GSI: {1} - '
        'Write provisioning will be decreased to {3:d} units'.format(
            table_name,
            index_name,
            updated_provisioning))
...
def increase_writes_in_percent(
    ...
    if get_table_option(key_name, 'gsi_max_provisioned_writes') > 0:
        if (updated_provisioning >
                get_table_option(key_name, 'gsi_max_provisioned_writes')):

            logger.info(
                '{0} - GSI: {1} - '
                'Reached provisioned writes max limit: {3:d}'.format(
                    table_name,
                    index_name,
                    int(get_table_option(
                        key_name, 'gsi_max_provisioned_writes'))))

            return get_table_option(key_name, 'gsi_max_provisioned_writes')

    logger.debug(
        '{0} - GSI: {1}'
        'Write provisioning will be increased to {3:d} units'.format(
            table_name,
            index_name,
            updated_provisioning))
...
def decrease_reads_in_units(
        current_provisioning, units, key_name, table_name, index_name):
    ...
    if min_provisioned_reads > 0:
        if updated_provisioning < min_provisioned_reads:
            logger.info(
                '{0} - GSI: {1} - '
                'Reached provisioned reads min limit: {3:d}'.format(
                    table_name,
                    index_name,
